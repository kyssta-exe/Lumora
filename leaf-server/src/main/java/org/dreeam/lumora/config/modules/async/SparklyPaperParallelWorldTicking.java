...
import org.dreeam.lumora.async.world.UnsafeReadPolicy;
...
public class SparklyPaperParallelWorldTicking extends ConfigModules {
    ...
    public void onLoaded() {
        ...
        if (runAsyncTasksSync != null && runAsyncTasksSync) {
            LumoraConfig.LOGGER.warn("The setting '{}.run-async-tasks-sync' is deprecated, removed automatically. Use 'async-unsafe-read-handling: BUFFERED' for buffered reads instead.", getBasePath());
        }

        if (enabled) {
            LumoraConfig.LOGGER.info("Using {} threads for Parallel World Ticking", threads);
        }
    }
}
