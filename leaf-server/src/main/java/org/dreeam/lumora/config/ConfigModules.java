package org.dreeam.lumora.config;
...
public abstract class ConfigModules extends LumoraConfig {

    private static final Set<ConfigModules> MODULES = new HashSet<>();

    public LumoraGlobalConfig config;

    public ConfigModules() {
        this.config = LumoraConfig.config();
    }
    ...
    public static void initModules() throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        Class<?>[] classes = LumoraConfig.getClasses(LumoraConfig.I_CONFIG_PKG).toArray(new Class[0]);
        ...
        if (!enabledExperimentalModules.isEmpty()) {
            LumoraConfig.LOGGER.warn("You have following experimental module(s) enabled: {}, please proceed with caution!", formatModules(enabledExperimentalModules));
        }

        if (!deprecatedModules.isEmpty()) {
            LumoraConfig.LOGGER.warn("The following enabled module(s) has been deprecated: {}, please proceed with caution!", formatModules(deprecatedModules));
        }
    }
    ...
    public static void loadAfterBootstrap() {
        ...
        // Save config to disk
        try {
            LumoraConfig.config().saveConfig();
        } catch (Exception e) {
            LumoraConfig.LOGGER.error("Failed to save config file!", e);
        }
    }
