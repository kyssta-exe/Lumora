...
public class FastRNG extends ConfigModules {
    ...
    public void onLoaded() {
        ...
        if (enabled) {
            try {
                Class.forName("org.dreeam.lumora.util.math.random.FasterRandomSource");
            } catch (Throwable ignored) {
                LumoraConfig.LOGGER.error("Faster random generator is enabled but Xoroshiro128PlusPlus is not supported by your JVM, " +
        ...
        if (enabled && warnForSlimeChunk) {
            LumoraConfig.LOGGER.warn("You enabled faster random generator, it will offset location of slime chunk");
            LumoraConfig.LOGGER.warn("If your server has slime farms or facilities need vanilla slime chunk,");
            LumoraConfig.LOGGER.warn("set performance.faster-random-generator.use-legacy-random-for-slime-chunk " +
                "to true to use LegacyRandomSource for slime chunk generation.");
            LumoraConfig.LOGGER.warn("Set performance.faster-random-generator.warn-for-slime-chunk to false to " +
